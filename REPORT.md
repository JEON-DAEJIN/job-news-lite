...
> 원래 프로젝트 자체의 계획·완성도·실사용 방법(과제와 무관하게)은
> [`프로젝트_보고서_및_사용설명서.md`](./프로젝트_보고서_및_사용설명서.md)에 따로 정리했습니다.

### 원래 계획(PRD v2) vs 과제 가이드라인 반영 후 — 무엇이 달라졌나

과제 제출 형식(`graph.py`/`audience.yaml`/`store/metrics.jsonl` 등)을 얹으면서 원래
계획에 없던 변화가 몇 가지 생겼습니다. 전부 "과제를 위해 억지로 넣은 것"은 아니고,
실제로 프로젝트에 도움이 된 것과, 설명 부담만 늘어난 것이 섞여 있습니다 — 정직하게
구분했습니다.

| 항목 | 원래 계획 (PRD v2, 과제 이전) | 과제 가이드라인 반영 후 | 평가 |
|---|---|---|---|
| 조건 설정 | 키워드·임계치가 `rules.py`/`main.py` 코드에 하드코딩 | `audience.yaml` 한 파일로 단일화 | **좋아짐** — 코드를 안 건드리고 조건을 바꿀 수 있게 됨(사용설명서 2.4절) |
| 소스 구성 | WebSearch 통합 1회 호출(트랙 구분 없음) | 3트랙(`public_agency`/`job_platform`/`exec_news`) 분리 | **좋아짐** — 어느 소스가 실제로 기여하는지 추적 가능(실제로 `public_agency`가 0건이라는 걸 알게 됨). **제약** — LLM 호출이 1회→3회로 늘어 실행 시간·비용이 늘어남 |
| 검수 | 한국어 필드 검사만 | + 마감일 검증 관문(`_is_past_deadline`) 추가 | **좋아짐** — 이번 실행에서 실제로 마감 지난 후보 1건을 걸러냄(N036에서 배운 패턴 반영) |
| 실행 기록 | 콘솔 출력뿐, 저장 안 됨(휘발성) | `store/metrics.jsonl`에 실행마다 영구 기록 | **좋아짐** — 나중에 "이 조건이 몇 건이나 걸렀는지" 추적 가능해짐(사용설명서 2.6절) |
| 프레임워크 | Claude Agent SDK로 자유롭게 설계 | 과제 예시가 LangGraph를 제시 → SDK를 유지하되 "왜 LangGraph가 아닌지"를 매번 설명해야 함 | **제약** — 실제 구현·성능엔 영향 없었지만, 문서화 부담이 늘어남(4장 콜아웃) |
| 발행 채널 | 로컬 HTML(원래 계획대로) | 그대로 유지, 대신 "목표 채널 발행 확인" 요건에 맞는지 설명해야 함 | **제약(설명 부담만)** — 실질적인 코드 변경은 없었음 |
| 문서 구조 | PRD + Handoff 두 문서만 | + `REPORT.md`(6장 형식, 이 문서) | **좋아짐** — 설계 판단이 명문화됨. **제약** — 과제 형식에 맞추다 보니 "실사용자 입장" 설명이 부족해져서, 별도로 `프로젝트_보고서_및_사용설명서.md`를 추가로 써야 했음 |
| 저장·공유 형태 | `SearchingJobs/` 저장소 하위의 미추적 폴더 | 독립 GitHub public 저장소(`job-news-lite`)로 분리 | **좋아짐** — 프로젝트가 독립적으로 관리·공유됨. **제약** — public이라 설계 판단이 전부 공개됨(민감정보 없음은 확인함) |

**요약**: 대부분은 실질적으로 프로젝트를 더 좋게 만들었습니다(`audience.yaml`, 3트랙 분리,
마감일 검증, 실행 로그가 전부 "과제가 없었어도 나중엔 필요했을" 개선입니다). 진짜 순수한
"제약"은 두 가지뿐입니다 — ① LangGraph/발행채널에 대해 매번 선택 근거를 설명해야 하는
문서화 부담, ② 소스를 3트랙으로 나누면서 실행 시간이 늘어난 것.
...
**2026-09-15, 트랙 분리 버전을 두 차례 실행**한 실제 기록(`store/metrics.jsonl` 원본):

| 트랙(source_id) | 1차 실행 (11:47 KST) | 2차 실행 (12:32 KST) |
|---|---|---|
| `public_agency` (정부·공공기관·대학) | 0건 | 0건 |
| `job_platform` (채용 플랫폼) | 2건 | 2건 |
| `exec_news` (임원 인사·영입 뉴스) | 2건 | 2건 |
| **합계 (중복 제거 후)** | **4건 (중복 0건)** | **4건 (중복 0건)** |
| **최종 발행** | 0건 | **2건 (일반기업)** |

두 실행 모두 `public_agency` 트랙이 0건입니다 — 워크넷 API를 아직 못 쓰는 상태에서
WebSearch만으로는 공공기관 채용 공고 자체를 찾기 어렵다는 걸 재확인시켜주며, 이는 2장의
"워크넷 API 탈락(이번 버전)" 판단이 실제로 이 트랙의 약점으로 이어졌음을 보여주는 근거입니다.
반면 같은 조건으로 다시 실행했을 때 최종 발행 건수는 0건→2건으로 달라졌습니다 — 채용
공고는 매 순간 바뀌므로 "한 번 0건이었다고 항상 0건은 아니다"라는 걸 보여주는 사례이기도
합니다. 자세한 원인 분석과 실제 발행 화면은 5장 "실행 기록"에 있습니다.
...
**2026-09-15 두 차례 실행 실제 기록**(`store/metrics.jsonl`):
...
// 2차 실행 (12:32 KST)
{
  "gate_input": 4, "gate_passed": 3,
  "gate_rejected_ids": ["hennymccoy-cbo-2026"],
  "skipped_expired_deadline": [],
  "below_threshold": 1,
  "final_passed": 2,
  "published_company": 2
}
...
1차 실행은 4건 중 2건이 키워드 게이트에서 탈락, 게이트를 통과한 2건 중 1건은 **마감일이
이미 지나** 검수 단계에서 탈락, 나머지 1건은 **관련도 점수가 임계치(60점) 미달**로 탈락 —
최종 발행 0건이었습니다. 세 가지 서로 다른 필터가 각각 실제로 후보를 걸러낸 것이 확인됩니다.

45분 뒤 2차 실행에서는 같은 조건으로도 실제 채용 공고 후보(헤니맥코이의 CCO 채용 등)가
게이트·마감일·점수 임계치를 **모두 통과해 최종 2건이 발행**됐습니다(실제 발행 화면은
5장 참고) — 같은 파이프라인이 검색 시점에 따라 진짜로 다른(그리고 정확한) 결과를 낸다는
것을 보여줍니다.
...
### 최종 실행 1차 — 트랙 분리 버전 (2026-09-15 11:47 KST, 최종 발행 0건)
...
**최종 발행 0건**이라는 결과 자체는 "실패"가 아니라 — 4건 수집 → 2건 게이트 탈락 → 남은
2건 중 1건은 마감일 초과, 1건은 관련도 점수 미달로, **검수 단계의 두 가지 서로 다른 관문이
각자 실제로 작동해서 나온 정직한 결과**입니다. PRD 4장이 요구한 "0건이면 리포트에 명확히
표시"도 정상 동작했습니다(이때 찍은 빈 리포트 화면은 45분 뒤 2차 실행 결과로 덮어써졌고,
아래 2차 실행이 실제 발행 화면입니다).

### 최종 실행 2차 — 같은 조건 재실행 (2026-09-15 12:32 KST, 최종 발행 2건)
...
[1/4] 채용 뉴스 검색 중... (소스 3갈래)
  ...
  - [채용 플랫폼 공고] 검색 중...
  - [임원 인사·영입 뉴스] 검색 중...
[2/4] 관문(Gate) 통과 확인 중... 4건 수집(중복 제거 후 4건) 중 3건 통과
[3/4] 관련도 평가 및 추천 이유(인사이트) 작성 중...
    검수 완료 — 마감 지남 0건 제외, 임계치 미달 1건 제외, 최종 2건
생성 완료: .../data/report_latest.html (12336 bytes)
[4/4] 리포트 생성 완료 — 브라우저를 엽니다
```

이번엔 실제 채용 후보 — 헤니맥코이(Henny McCoy)의 CCO(Chief Content Officer) 채용
공고(인크루트 원문, 마감 2026-09-18) — 가 게이트·마감일·점수 임계치(90점)를 전부 통과해
**일반기업 카테고리에 2건 발행**됐습니다. TOP 1 추천 이유도 confirmed_facts(20년 이상
경력 요구, IP/플랫폼/엔터 콘텐츠전략 총괄 등)에만 근거해 작성된 것을 원문 대조로 확인했습니다.

**실제 라이브 실행 결과 화면 캡처** (`docs/assets/execution_live_2026-09-15.png`,
2차 실행 결과 — 더미 데이터 아님):

![실제 라이브 실행 결과 — 헤니맥코이 CCO 채용 공고가 TOP 1로 발행된 화면](./docs/assets/execution_live_2026-09-15.png)

### 남은 한계

- `public_agency`(정부·공공기관) 트랙은 두 실행 모두 0건이었습니다 — 워크넷 API 없이
  WebSearch만으로는 공공기관 채용 공고 자체를 찾기 어려웠다는 뜻으로 보이며, 2장 "향후
  보완"에 반영했습니다.
- 표본이 두 차례(4건씩)뿐이라 "게이트/점수/마감일 필터가 통계적으로 얼마나 정확한지"까지는
  이번 실행만으로 증명되지 않습니다 — 여러 날에 걸쳐 반복 실행해야 신뢰도가 쌓입니다.
