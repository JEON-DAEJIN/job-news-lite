...
`collected_by_source`에 남깁니다. 최초 실행(2026-09-14, 2주 창) 결과:

| 트랙 | 수집 | 게이트 통과 |
|---|---|---|
| (통합 검색, 트랙 분리 이전 버전) | 0건 | 0건 |

이 결과 자체가 "실제로 검증"의 일부입니다 — 자세한 원인 분석과 대응은 5장 "실행 기록"에
있습니다(요약: 기본 파라미터로는 검색→목록 페이지→개별 공고 열람까지 끝내기엔 턴/시간이
부족했고, 파라미터를 늘리자 실제 후보가 확인됨). 트랙 분리 버전의 최신 실행 결과는 5장에
갱신됩니다.
...
미달 건수), `final_passed`. 실제 수치는 5장에 있습니다.
...
> **참고**: 이 절은 실제 라이브 실행 결과로 채웁니다. 이 리포트를 작성하는 시점에 Claude
> Code CLI 세션 사용량 한도(`session limit · resets 7:10pm Asia/Seoul`)에 걸려, 최종
> 실행·발행 확인은 한도가 풀린 뒤 완료했습니다 — 아래는 그 과정의 정직한 기록입니다.

### 시도 1 — 검색 기간 2주, 기본 파라미터
...
→ `graph.py`의 수집 단계 파라미터를 이 값으로 고정했습니다.

### (한도 해제 후 채울 항목)

- [ ] 3트랙(공공기관/채용플랫폼/인사뉴스) 각각의 수집 건수
- [ ] `dedup_candidates` 중복 제거 건수
- [ ] 게이트 통과/탈락 건수와 탈락 사유
- [ ] 최종 발행 건수 및 `store/metrics.jsonl` 실제 레코드
- [ ] 정상 발행된 리포트 화면 캡처
